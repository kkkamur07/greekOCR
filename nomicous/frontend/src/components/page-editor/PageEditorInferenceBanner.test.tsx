...
describe('PageEditorInferenceBanner', () => {
  it('shows the compact banner (not a blocking modal) when helper is unavailable', () => {
    ...
    expect(screen.queryByRole('dialog', { name: /install inference helper/i })).toBeNull();
    expect(screen.getByRole('button', { name: /install helper/i })).toBeTruthy();
  });
  ...
  it('opens the install modal only after clicking install helper', () => {
    ...
    expect(screen.getByRole('dialog', { name: /install inference helper/i })).toBeTruthy();
  ...
  it('calls onUseCloudInstead from the modal', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /install helper/i }));
    fireEvent.click(screen.getByRole('button', { name: /use cloud inference instead/i }));
