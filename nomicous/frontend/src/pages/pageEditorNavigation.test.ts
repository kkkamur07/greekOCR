...
describe("readPageEditorDocument", () => {
  it("returns the document when route ids match", () => {
    expect(
      readPageEditorDocument({ document: DOCUMENT }, "proj-1", "doc-1"),
    ).toBe(DOCUMENT);
  });

  it("returns null when ids do not match", () => {
    expect(
      readPageEditorDocument({ document: DOCUMENT }, "proj-2", "doc-1"),
    ).toBeNull();
    expect(
      readPageEditorDocument({ document: DOCUMENT }, "proj-1", "doc-2"),
    ).toBeNull();
