...
describe("PageEditorPlaceholderPage shell", () => {
  ...
  it("opens an authenticated document part in the annote page workspace", async () => {
    ...
    expect(
      screen
        .getByRole("link", { name: /back to document/i })
        .getAttribute("href"),
  ...
  it("renders classic strip layout with toolbar, canvas, and pairing strip when a Segment is selected", async () => {
    ...
    expect(
      screen.getByRole("group", { name: /page geometry canvas/i }),
    ).toBeTruthy();
    ...
    expect(
      screen.getByRole("progressbar", { name: /pairing progress/i }),
    ).toBeTruthy();
  });
  ...
  it("opens the transcription PDF side pane from the toolbar", async () => {
    ...
    expect(await screen.findByText("ANNOTE PAGE WORKSPACE")).toBeTruthy();
    fireEvent.click(
      screen.getByRole("button", { name: /toggle transcription pdf/i }),
    );

    expect(
      await screen.findByLabelText("Transcription PDF preview"),
    ).toBeTruthy();
    await waitFor(() => {
  ...
  it("publishes the document from the Process sharing section", async () => {
    ...
    await waitFor(() => {
      expect(mockedApi.updateDocument).toHaveBeenCalledWith(
        "project-1",
        "doc-1",
        {
          workflow: "published",
