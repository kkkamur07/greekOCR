...
describe("API auth recovery", () => {
  ...
  it("refreshes and retries a protected JSON request once", async () => {
    ...
    await expect(
      apiRequest<{ id: string }>("/projects/project-1"),
    ).resolves.toEqual({
      id: "project-1",
    });
  ...
  it("does not refresh skipAuth routes", async () => {
    ...
    await expect(
      apiRequest("/public/projects/one", { skipAuth: true }),
  ...
  it("keeps abortable GET requests independent", async () => {
    ...
    const first = apiRequest<{ source: string }>(
      "/projects/project-1/jobs?limit=8",
      {
        signal: firstController.signal,
      },
    );
    const second = apiRequest<{ source: string }>(
      "/projects/project-1/jobs?limit=8",
      {
