...
def upgrade() -> None:
    ...
    op.add_column("jobs", sa.Column("ml_job_id", sa.UUID(), nullable=True))
    op.create_index(op.f("ix_jobs_ml_job_id"), "jobs", ["ml_job_id"], unique=False)


def downgrade() -> None:
    op.drop_index(op.f("ix_jobs_ml_job_id"), table_name="jobs")
    op.drop_column("jobs", "ml_job_id")
