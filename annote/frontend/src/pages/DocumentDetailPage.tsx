...
export function DocumentDetailPage() {
  ...
  const [reviewUpdatingPartId, setReviewUpdatingPartId] = useState<string | null>(null);
  ...
  const handleToggleReview = async (partId: string, reviewed: boolean) => {
    ...
    try {
      await api.updatePartReviewStatus(projectId, documentId, partId, { reviewed });
      toast.success(reviewed ? 'Part marked reviewed' : 'Part marked unreviewed');
      await load();
    } catch (err) {
      const msg =
  ...
  return (
    <AppPageShell
      breadcrumb={[
        ...
        { label: projectName ?? 'Project', href: projectId ? `/projects/${projectId}` : undefined },
        { label: document?.name ?? 'Document' },
      ]}
      username={username}
      title={document?.name ?? 'Document'}
      subtitle={subtitle}
      titleExtra={document ? <WorkflowBadge workflow={document.workflow} /> : undefined}
    ...
    >
      ...
      {document && (
        <UploadZone onUpload={handleUpload} disabled={loading} loading={uploading} />
      )}
      ...
      {document && (
        <>
          ...
          <PartList
            ...
            onToggleReview={(partId, reviewed) => void handleToggleReview(partId, reviewed)}
