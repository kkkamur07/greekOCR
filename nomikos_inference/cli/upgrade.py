...
from nomikos_inference.cli.credentials import CredentialError, load_credential
...
def _re_exec(previous_version: str) -> None:
    ...
    if script and os.path.isfile(script) and os.access(script, os.X_OK):
        ...
        # Started as `python -m nomikos_inference.cli`, or through something that
        # left no usable `argv[0]`. The module path resolves to the new code
        # just as well.
        script = sys.executable
        arguments = [sys.executable, "-m", "nomikos_inference.cli", *sys.argv[1:]]
