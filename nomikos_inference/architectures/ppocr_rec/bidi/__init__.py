...
Vendored from kraken 7.1.1, kraken/lib/bidi, Apache License 2.0; reformatted with ruff, algorithm unchanged.
"""
...
from nomikos_inference.architectures.ppocr_rec.bidi._data import (
    BIDI_CLASS_RANGES,
    BRACKETS,
    MIRRORED,
    UCD_VERSION,
...
def _match_isolates(types: list[str]) -> tuple[dict[int, int | None], set[int]]:
    """
...
def _explicit_levels(
    types: list[str], wtypes: list[str], matching_pdi: dict[int, int | None], para_level: int
) -> list[int | None]:
    """
    ...
    ``wtypes`` in place.
    """
    levels: list[int | None] = [para_level] * len(types)
    ...
    for idx, typ in enumerate(types):
        if typ in _EXPLICIT_EMBEDDINGS:
            ...
            if typ == "FSI":
                ...
                rtl = (
                    _first_strong_level(types, idx + 1, pdi if pdi is not None else len(types)) == 1
                )
            else:
                rtl = typ == "RLI"
...
def _isolating_run_sequences(
    ...
    keep: list[int],
    matching_pdi: dict[int, int | None],
...
def _n0_strong(typ: str) -> str | None:
...
def _resolve_neutrals(
    seq: list[int], sos: str, eos: str, wtypes: list[str], embedding_dir: str
) -> None:
    """
...
def resolve_levels(
    types: list[str], cps: list[int] | None, para_level: int
    ...
    for seq, sos, eos in _isolating_run_sequences(
        types, levels, keep, matching_pdi, matched_pdis, para_level
    ):
    ...
    if visual:
        ...
        lowest_odd = min((levels[pos] for pos in visual if levels[pos] % 2), default=None)
...
def get_display_map(text: str, base_dir: Literal["L", "R"] | None = None) -> tuple[str, list[int]]:
    """
...
def get_display(text: str, base_dir: Literal["L", "R"] | None = None) -> str:
    """
    Reorders a string from logical order into display order according to the
    Unicode 17.0.0 bidirectional algorithm. The text is treated as a single
    paragraph.
