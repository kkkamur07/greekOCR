...
def build_calamari_train_command() -> tuple[list[str], dict[str, str]]:
    """Invoke the supported Calamari distribution, not a vendored copy."""
    if importlib.util.find_spec("calamari_ocr") is None:
        raise ModuleNotFoundError(
            "Calamari is not installed. Run with the 'train-calamari' extra."
        )
    return [sys.executable, "-m", "calamari_ocr.scripts.train"], os.environ.copy()
