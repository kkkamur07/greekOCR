...
> **Status: archived** — original eScriptorium-style platform PRD. Implementation lives in `nomicous/`; see `done/prd-annote-merge.md` and `nomicous/CONTEXT.md`.
...
- **Shipped:** Postgres `NOTIFY` on `jobs` status changes + SSE to the browser — see [root README](../../README.md#job-status-sse-not-polling).
