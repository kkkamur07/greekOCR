...
_Avoid_: dev weights, ml_service/weights
...
_Avoid_: runtime weight cache, ml_service/weights/cache
...
**Hub integration**:
Python code under `src/hf/` that resolves `hf://` URIs, checks **Hub cache**, and downloads missing artifacts. Used by `ml_service` at inference time and by `scripts/hf/`.
...
Hub upload and fetch tooling lives under repo-root `scripts/hf/`. All Hub-related data and integration code lives under `src/hf/` (**Hub staging tree**, **Hub cache**, **Hub integration**). Repo-operational scripts consolidate under root `scripts/` by domain.
