...
from ml_service.infrastructure.db import JobNotificationListener, SessionLocal
from ml_service.infrastructure.job_repository import (
...
from ml_service.jobs.worker import run_worker, wait_for_worker_schema
...
def test_wait_for_worker_schema_raises_when_table_never_appears(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr("ml_service.jobs.worker.ml_jobs_table_ready", lambda: False)
...
def test_wait_for_worker_schema_returns_after_table_appears(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("ml_service.jobs.worker.ml_jobs_table_ready", ready)
...
def _submit(product_job_id=None) -> MLJob:
    return create_job(
        JobSubmitRequest(
            ...
            image_bytes=SEGMENT_IMAGE_PATH.read_bytes(),
        )
    )
...
def test_worker_recovers_stale_running_job_without_notification(
    ...
    monkeypatch.setattr("ml_service.jobs.worker.get_ml_settings", lambda: settings)
    monkeypatch.setattr("ml_service.infrastructure.db.get_ml_settings", lambda: settings)
