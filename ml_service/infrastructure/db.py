...
class JobNotificationListener:
    """Blocking LISTEN/NOTIFY helper for waking idle ML workers."""

    def __init__(self, channel: str | None = None) -> None:
        self.channel = channel or get_ml_settings().worker_notify_channel
        self._connection = None
        self._cursor = None

    def __enter__(self) -> JobNotificationListener:
        self._connection = psycopg2.connect(get_ml_settings().database_url)
...
def ensure_schema() -> None:
    from ml_service.infrastructure.orm_models import MLJob  # noqa: F401
